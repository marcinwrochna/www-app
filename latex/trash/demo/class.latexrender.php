    ...
    function getFormulaURL($latex_formula) {
	    ...
	    while ($file = readdir($handler)) {
			if (Substr($file,0,32).Substr($file,strlen($file)-4,4)==Substr($filename,0,32).Substr($filename,strlen($filename)-4,4)) {
				if ($this->_cachefiles == 0) {
					unlink($this->getPicturePath()."/".$file);
				} else {
					$filename = $file;
					$full_path_filename = $this->getPicturePath()."/".$filename;
				}
...
//       if ((is_file($full_path_filename)) && ($this->_cachefiles == 0)) {
...
//		}
        ...
        if (is_file($full_path_filename)) {
            ...
            if (strlen($latex_formula) > $this->_string_length_limit) {
            	$this->_errorcode = 1;
            	$this->_errorextra = ": Formula length is ".strlen($latex_formula)." which is longer than ".$this->_string_length_limit." characters";
                return false;
            }
            ...
            for ($i=0;$i<sizeof($this->_latex_tags_blacklist);$i++) {
                if (stristr($latex_formula,$this->_latex_tags_blacklist[$i])) {
                	$this->_errorcode = 2;
                	$this->_errorextra = ": Includes a blacklisted tag <i>".$this->_latex_tags_blacklist[$i]."</i>";
                    return false;
                }
            }
            ...
            if ($this->renderLatex($latex_formula)) {
                ...
                // $this->_errorcode = 3;
                // $this->_errorextra = ": Latex rendering failed";
                return false;
            }
        }
    }
    ...
    function renderLatex($latex_formula) {
        ...
        if (!$status_code) { $this->cleanTemporaryDirectory(); chdir($current_dir); $this->_errorcode = 4; $this->_errorextra = ": Cannot create DVI file"; return false; }
        ...
        if ( ($dim["x"] > $this->_xsize_limit) or ($dim["y"] > $this->_ysize_limit)) {
            ...
            chdir($current_dir);
            $this->_errorcode = 5;
            $this->_errorextra = ": Image is " . $dim["x"] . " x " . number_format($dim["y"],0,"","").
            " Maximum size is ".$this->_xsize_limit . " x " . $this->_ysize_limit;
            return false;
        }
        ...
        if (!$status_code) { chdir($current_dir); $this->_errorcode = 6; $this->_errorextra = ": Cannot copy image to pictures directory";return false; }
        chdir($current_dir);
