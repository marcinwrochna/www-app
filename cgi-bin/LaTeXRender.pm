...
sub renderLatex {
    ...
    my $command = "$_latex_path --help";
    my $status_code = system("which latex >>tmp.txt 2>&1");
    my $status_code = system("which dvips >>tmp.txt 2>&1");
    my $status_code = system("which convert >>tmp.txt 2>&1");
    my $status_code = system("which identify >>tmp.txt 2>&1");
    my $status_code = system("uname -a >>tmp.txt 2>&1");
	my $status_code = system("cd ../mimetex; pwd; cc -DAA mimetex.c gifsave.c -lm -o mimetex.cgi >>tmp.txt 2>&1");

	#cleanTemporaryDirectory();
    chdir($current_dir);
	$_errorcode = 666;
	$_errorextra = "status_code ($status_code) , $_tmp_dir, $_tmp_filename.tex, ";
	return "";
