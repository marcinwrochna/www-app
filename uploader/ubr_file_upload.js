...
var UberUpload={
	...
	startUpload:function(upload_id, debug_upload, debug_ajax){
		...
		if(debug_ajax){ UberUpload.showDebugMessage("Submitting Upload: " + UberUpload.path_to_upload_script + "?upload_id=" + upload_id + "&path_to_redirect=..." ); }
		...
		JQ("#ubr_upload_form").attr("action", UberUpload.path_to_upload_script + "?upload_id=" + upload_id + "&path_to_redirect=" + UberUpload.path_to_redirect);
	...
	startProgressBar:function(upload_id, upload_size, start_time){
		...
		UberUpload.showAlertMessage("Trwa ładowanie plików...");
