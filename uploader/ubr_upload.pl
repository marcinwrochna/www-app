...
if($DEBUG_PERL){
	...
	if(exists($query_string{'path_to_redirect'})){ $PATH_TO_REDIRECT = $query_string{'path_to_redirect'}; }
	else{ &kak("Invalid upload id", 1, __LINE__); }
...
$config{'upload_dir'} .= $query->param("myuploaddir");
$config{'path_to_upload'} .= $query->param("myuploaddir");
...
if($config{'redirect_after_upload'} > 0){
	...
	my $redirect_url = $PATH_TO_REDIRECT . "?upload_id=" . $UPLOAD_ID;
	...
	print "content-type:text/html; charset=utf-8\n\n";
	print "<script language=\"javascript\" type=\"text/javascript\">UbrUploadDialog.insertUpload();</script>\n";
}
